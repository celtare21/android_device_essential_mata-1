...
patchelf --add-needed "libbase_shim.so" "$AICP_ROOT"/vendor/"$VENDOR"/"$DEVICE"/proprietary/vendor/lib/lib-imsrcs-v2.so
patchelf --add-needed "libbase_shim.so" "$AICP_ROOT"/vendor/"$VENDOR"/"$DEVICE"/proprietary/vendor/lib/lib-uceservice.so
